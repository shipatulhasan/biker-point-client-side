...
const MyOrders = () => {
    ...
    useEffect(()=>{
        setIsLoading(true)
        axios.get(`${process.env.REACT_APP_api}/booking?email=${user?.email}`,{
            headers:{
    ...
    const handleDelete = (booking)=>{
      ...
      const confirmcancle = window.confirm(`would you like to cancel ${booking.product_title}`)
      if(confirmcancle){

        fetch(`${process.env.REACT_APP_api}/booking/${booking?._id}`,{
          method:'delete',
          headers:{
            authorization:`Bearer ${localStorage.getItem('biker-point-token')}`
        }
        ...
        <div className='px-4 sm:px-8 py-4 overflow-x-auto'>
          {
            bookings?.length===0 ? <p className='text-black text-2xl font-bold'>No booking booked yet. wanna booke some products ? <Link to='/categories' className='text-red-600 font-semibold '>Click here</Link></p>
             ...
             <div className='min-w-full shadow rounded-lg overflow-x-auto'>
             {
               ...
               <tbody>
                   {
                     bookings.map((booking,i)=><OrdersRaw key={booking._id} booking={booking} index={i} handleDelete={handleDelete} />)
                   }
