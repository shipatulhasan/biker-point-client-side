...
const ReportedProduct = () => {
  ...
  const {data:products,refetch,isLoading} = useQuery({
    ...
    queryFn:async()=>{
            ...
            const res= await fetch(`${process.env.REACT_APP_api}/reported-product`,{
                headers:{
    ...
    const handleDelete = (product)=>{
      ...
      if(confirmDelete){
        ...
        fetch(`${process.env.REACT_APP_api}/reported-product/${product?._id}`,{
          method:'delete',
          headers:{
            authorization:`Bearer ${localStorage.getItem('biker-point-token')}`
        }
...
<Helmet>
      ...
      <div className='px-4  py-4 overflow-x-auto'>
        {
          products?.length< 1 ? <p className='text-black text-2xl font-bold'>No Reported item Found.</p>
           :
          <>
           {
             isLoading ? <Loader /> : <>
             ...
             <tbody>
                 {
                   products.map((product,i)=><ItemRow key={product._id} product={product} index={i} handleDelete={handleDelete}/>)
                 }
