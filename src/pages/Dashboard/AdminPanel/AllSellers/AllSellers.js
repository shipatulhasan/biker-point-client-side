...
const AllSellers = () => {
    ...
    const handleDelete = (seller)=>{
      ...
      const confirmDelete = window.confirm(`would you like to delete ${seller?.email}`)
      if(confirmDelete){

        fetch(`${process.env.REACT_APP_api}/user/${seller?._id}`,{
          method:'delete',
          headers:{
    ...
    const handleVerification = (seller)=>{
      ...
      delete seller._id
        ...
        const confirmation = window.confirm(`would you like to verify ${seller?.email}`)

        if(confirmation){
            fetch(`${process.env.REACT_APP_api}/user/${seller?.email}`,{
                method:'put',
                headers:{
    ...
    return (
        <div className='container mx-auto px-4 sm:px-8 py-8'>
          ...
          <div className='min-w-full shadow rounded-lg'>
              ...
              {
                  users.map((seller,i)=><SellerInfo key={seller._id} seller={seller} index={i} handleDelete={handleDelete} handleVerification={handleVerification}/>)
                }
